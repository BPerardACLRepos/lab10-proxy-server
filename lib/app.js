...
app.get('/location', async (req, res) => {
  try {
    ...
    const responseCity = responseData[0];

    const formattedLocation = getLocation(responseCity);
...
app.get('/weather', async (req, res) => {
  try {
    ...
    const responseForecasts = responseData.slice(0, 5);

    const formattedForecasts = getLocation(responseForecasts);
...
app.get('/weather', async (req, res) => {
  try {
    ...
    const responseData = await request.get(`https://api.yelp.com/v3/businesses/search?latitude=${queryLat}&longitude=${queryLon}`)
      .set('Authorization', `Bearer ${process.env.YELP_API_KEY}`);

    const responseReviews = responseData.slice(0, 5);

    const formattedReviews = getLocation(responseReviews);
